StatAccidentDeathCtrl.$inject = ['$rootScope', '$scope', 'IHttp', 'i18nService', 'uiGridConstants', '$timeout', 'toastr'];

module.exports = StatAccidentDeathCtrl;

function StatAccidentDeathCtrl( $rootScope, $scope, IHttp, i18nService, uiGridConstants, $timeout, toastr) {
	...
	$scope.$on('query',function(ev, data){
		...
		IHttp.post('statistics/searchPatGroupByDept', param).then((rs) => {
		    if (rs.status === 200 && rs.data.resultCode === '1') {
	    	    ...
	    	    $scope.gridOptions.columnDefs = diedCountlist;
