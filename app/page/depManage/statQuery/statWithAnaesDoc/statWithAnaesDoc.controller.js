StatWithAnaesDocCtrl.$inject = ['$rootScope', '$scope', 'IHttp', 'i18nService', 'uiGridConstants', '$timeout', 'toastr'];

module.exports = StatWithAnaesDocCtrl;

function StatWithAnaesDocCtrl( $rootScope, $scope, IHttp, i18nService, uiGridConstants, $timeout, toastr) {
	...
	$scope.gridOptions = {
	    ...
	    exporterCsvFilename: '麻醉医师麻醉例数统计.csv',
	...
	$scope.$on('query',function(ev, data){
		...
		IHttp.post('statistics/searchAnaesMethodByAnaesDoc', data).then((rs) => {
		    if (rs.status === 200 && rs.data.resultCode === '1') {
		    	...
		    	for (let i = 0; i <= columnArr.length; i++) {
		    	    if (i === 0) {
		    	        ...
		    	        columns[i] = {
		    	            field: columnArr[i - 1],
		    	            displayName: rs.data.timeTitle[i - 1]
		    	        };
		    	    }
		    	}
