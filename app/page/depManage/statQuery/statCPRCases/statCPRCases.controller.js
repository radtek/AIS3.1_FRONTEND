StatCPRCasesCtrl.$inject = ['$rootScope', '$scope', 'IHttp', 'i18nService', 'uiGridConstants', '$timeout', 'toastr'];

module.exports = StatCPRCasesCtrl;

function StatCPRCasesCtrl( $rootScope, $scope, IHttp, i18nService, uiGridConstants, $timeout, toastr) {
	...
	function setEchartOption(data) {
		if (!data.dataStat.xAxis)
		    return;
		$scope.eConfig.dataLoaded = false;

		var len = data.dataStat.xAxis[0].data.length * data.dataStat.series[0].length;
		if (len >= 50 && len <= 150)
		...
		$scope.eOption.xAxis = data.dataStat.xAxis[0];
		$scope.eOption.yAxis = data.dataStat.yAxis[0];
		$scope.eOption.yAxis.name = '例数';
		if (len < 50) {
		    $scope.eOption.series = (function() {
		        var res = [];
		        data.dataStat.series.map(function(data) {
		            data.barWidth = 15;
		            res.push(data);
		        })
		        return res;
		    ...
		    $scope.eOption.series = data.dataStat.series;
		}
	}
	...
	$scope.gridOptions = {
	    ...
	    exporterCsvFilename: '麻醉医师实施心肺复苏治疗例数统计.csv',
	...
	$scope.$on('query',function(ev, data){
		IHttp.post('statistics/searchOperByCPRNum', data).then((rs) => {
