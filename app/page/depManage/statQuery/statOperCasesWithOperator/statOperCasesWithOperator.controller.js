StatOperCasesWithOperatorCtrl.$inject = ['$rootScope', '$scope', 'IHttp', 'i18nService', 'uiGridConstants', '$timeout', 'toastr'];

module.exports = StatOperCasesWithOperatorCtrl;

function StatOperCasesWithOperatorCtrl( $rootScope, $scope, IHttp, i18nService, uiGridConstants, $timeout, toastr) {
	...
	$scope.gridOptions = {
	    ...
	    exporterCsvFilename: '手术医生-手术例数统计.csv',
	...
	$scope.$on('query',function(ev, data){
		IHttp.post('statistics/searchOperByOperator', data).then((rs) => {
		    if (rs.status === 200 && rs.data.resultCode === '1') {
		    	...
		    	for (let i = 0; i < columnArr.length; i++) {
		    	    let column = {
		    	        field: columnArr[i],
		    	        displayName: rs.data.time[i]
		    	    };
		    	    columns[i + 1] = column;
		    	}

		    	$scope.gridOptions.columnDefs = columns;
		    	$scope.gridOptions.data = rs.data.voTableList;
		    	rs.data.xAxis = rs.data.xAxis;
		    	rs.data.yAxis = rs.data.yAxis;
		    	rs.data.series = rs.data.series;
