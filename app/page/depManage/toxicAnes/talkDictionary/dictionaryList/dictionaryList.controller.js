dictionaryListCtrl.$inject = ['$rootScope', '$scope', 'IHttp', '$timeout', '$filter', 'dateFilter', 'toastr', '$uibModal', 'auth', 'uiGridServe', 'select'];

module.exports = dictionaryListCtrl;

function dictionaryListCtrl($rootScope, $scope, IHttp, $timeout, $filter, dateFilter, toastr, $uibModal, auth, uiGridServe, select) {
    ...
    $scope.piliang = function() {
        ...
        IHttp.post('basedata/batAddMedicineOutRecord', { "receiveName": $scope.receiveName, "outTime": new Date($filter('date')(new Date($scope.outTime), 'yyyy-MM-dd HH:mm')).getTime(), "operator": user.userName, "outType":"1", "resList": rowArr }).then(function(rs) {
