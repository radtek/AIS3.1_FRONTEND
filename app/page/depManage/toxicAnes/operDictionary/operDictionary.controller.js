...
function operDictionaryCtrl( $rootScope, $scope, IHttp, i18nService,confirm,auth,$filter, uiGridConstants, $timeout,select,$q, toastr, $uibModal) {
	...
	function getPage() {
        var startTime=$scope.startTime;
            ...
            $scope.params.filters=[{"field":"outMedicine","value":"0"},{"field":"startTime","value":startTime},{"field":"endTime","value":endTime}];
    ...
    function getPage2() {
        setTimeout(function(){
            var startTime=$scope.startTime;
            var endTime=$scope.endTime;
            $scope.params2.filters=[{"field":"outMedicine","value":"1"},{"field":"startTime","value":startTime},{"field":"endTime","value":endTime}];
        ...
        IHttp.post('basedata/queryMedicineOutRecordList',$scope.params3).then(function(rs) {
            ...
            if (rs.status === 200 && rs.data.resultCode === '1') {
                ...
                for (var i = 0; i < rs.data.resultList.length; i++) {
                    rs.data.resultList[i].outTime=$filter('date')(rs.data.resultList[i].outTime, 'yyyy-MM-dd');
                }
    ...
    $scope.showIn = function(row){
        vm.regOptId = row.entity.regOptId;
        //查询详情
