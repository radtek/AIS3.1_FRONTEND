talkoperDictionaryEditCtrl.$inject = ['$rootScope', '$scope','confirm', 'IHttp','i18nService','uiGridConstants','$timeout','toastr', '$uibModalInstance', 'select', '$filter', 'auth'];

module.exports = talkoperDictionaryEditCtrl;
                               
function talkoperDictionaryEditCtrl($rootScope, $scope,confirm,  IHttp,i18nService,uiGridConstants,$timeout,  toastr, $uibModalInstance, select, $filter, auth) {
    vm = this;
    vm.title="退耗材";
        ...
        columnDefs: [{
                ...
                // },
                ...
                },
                 {
                    ...
                    enableSorting: false,
                    cellTemplate: '<div class="ui-grid-cell-contents" ><a class="hidden" href="" ng-click=grid.appScope.editIn(row)>编辑<span>|</span></a><a href="" ng-click=grid.appScope.deleteIn(row)>删除</a></div>'
                }]
    };

    function getPage() {
        IHttp.post('basedata/queryConsumablesOperationRetreatRecordList',$scope.params).then(function(rs) {
    ...
    $scope.editIn = function(row) {
        ...
        //回写
        $scope.medicalParams =row.entity;
    ...
    $scope.save = function(type) {
        ...
        IHttp.post('basedata/addConsumablesRetreatRecord', $scope.medicalParams)
            .then((rs) => {
