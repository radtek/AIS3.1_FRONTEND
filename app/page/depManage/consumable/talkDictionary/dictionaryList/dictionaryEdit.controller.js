...
function dictionaryEditCtrl($rootScope, $scope, IHttp, toastr, $uibModalInstance, select, dateFilter, $filter, auth) {
    ...
    $scope.save = function(type) {
       ...
       // $scope.medicalParam.effectiveTime=$scope.medicalParam.effectiveTime;
        $scope.medicalParam.outTime = new Date($filter('date')(new Date($scope.medicalParams.outTime), 'yyyy-MM-dd HH:mm')).getTime();
        ...
        IHttp.post('basedata/addConsumablesOutRecord', $scope.medicalParam)
            .then((rs) => {
    ...
    // 验证
    function verify() {
        return $scope.BaseInfoForm.$valid && !!($scope.medicalParams.outNumber && $scope.medicalParams.receiveName && $scope.medicalParams.outTime)
    }
}
