...
function talkDictionaryListCtrl($rootScope, $scope, confirm, IHttp, i18nService, uiGridConstants, $timeout, $filter, toastr, $uibModal, auth, select) {
    ...
    function getPage() {
        IHttp.post('basedata/queryConsumablesOutRecordList', $scope.params).then(function(rs) {
            if (rs.status === 200 && rs.data.resultCode === '1') {
                ...
                for (var i = 0; i < data.length; i++) {
                    ...
                    //1 普通取耗材，2 手术取耗材
                    if (data[i].outType === '1') {
                        data[i].outType1 = "普通取耗材";
                    } else if (data[i].outType === '2') {
                        data[i].outType1 = "手术取耗材";
                    }
    ...
    $scope.piliang = function() {
        ...
        IHttp.post('basedata/batAddConsumablesRetreatRecord', { "retreatName": $scope.retreatName, "retreatreason": "", "operator": user.userName, "resList": rowArr }).then(function(rs) {
