...
function AnesRecordLogCtrl($rootScope, $scope, IHttp, baseConfig, anesRecordInter, anesRecordServe, eCharts, auth, $filter, $q, $timeout, toastr, $uibModal, select, confirm) {
    ...
    if (vm.docInfo.beCode === 'syzxyy' || vm.docInfo.beCode == 'cshtyy'|| vm.docInfo.beCode == 'llzyyy') {
    ...
    anesRecordInter.startOper(regOptId, vm.pageState).then(function(rs) {
        ...
        // var xdfd=vm.startOper.leftShowList.pop();
        // vm.startOper.leftShowList.unshift(xdfd);//心电图排序处理
        // eCharts.option('mz', vm.startOper.anaesMedEvtList, ev_list, vm); // 麻醉用药情况瞄点数据生成
        // eCharts.option('zl', vm.startOper.treatMedEvtList, ev_list, vm); // 治疗用药情况瞄点数据生成
        // eCharts.option('sy', vm.startOper.inIoeventList, ev_list, vm); // 输液输血情况瞄点数据生成
        // eCharts.option('cl', vm.startOper.outIoeventList, ev_list, vm); // 出量情况瞄点数据生成
        if ((vm.docInfo.beCode == 'syzxyy' || vm.docInfo.beCode == 'cshtyy'|| vm.docInfo.beCode == 'llzyyy') && vm.operState != '04' && vm.docInfo.roleType != 'ANAES_DIRECTOR')
    ...
    function searchAllEventListPlus(rs) {
        anesRecordInter.searchAllEventListPlus("", { docId: rs ? rs.param.docId : docId, anaesMedNum: 0, medEventNum: 0, infusionNum: 0, egressNum: 0 }).then(function(rs) {
    ...
    vm.anesEvent = function() { // 麻醉事件
        IHttp.post('operation/searchApplication', { regOptId: regOptId }).then(function(rs) {
            ...
            $uibModal.open({
                ...
                eCharts.initSign1(vm.markEOpt1, vm.startOper, vm.view.pageSize, false, vm); // 更新标记
            });
        });
    }
    ...
    vm.onPrint = function() { // 打印
        ...
        }else if(vm.docInfo.beCode == 'llzyyy'){
             url = 'anesRecordPrint_llzyyy';
        }
