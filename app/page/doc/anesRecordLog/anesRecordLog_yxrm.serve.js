    ...
    let self = {
            ...
            IHttp.post("operation/searchAllEventList", { docId: docId, bloodNum: bloodNum, infusionNum: infusionNum, medEventNum: medEventNum, egressNum: egressNum, anaesMedNum: anaesMedNum, anaesEvtNum: anaesEvtNum, startTime: startTime, endTime: endTime }).then(function(result) {
                ...
                for (var i in res) {
                    ...
                    for (var j = 0; j < arr.length; j++) {
                        if (i == 'ctlBreath') {
                            list.unshift({ eventName: i, type: arr[j].type, startTime: arr[j].startTime });
                            continue;
                        }
                        if (i == 'otherevent') {
                        ...
                        if (i == 'sepecialMaterialEvent') {
                            list.push({ eventName: i, specialItemName: arr[j].specialItemName, startTime: arr[j].startTime });
                            continue;
                        }
                        if (i == 'shiftChange') {
        ...
        checkInput: function(event, startOper) {
            ...
            // } else if (event && !event.ssjs) {
                // return '手术结束时间不能为空';
            // } else 
            if (!startOper.anaesRecord.leaveTo) {
                return '出室去向不能为空';
