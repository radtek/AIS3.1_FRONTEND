...
function AnesRecordLogCtrl($rootScope, $scope, IHttp, baseConfig, anesRecordInter, anesRecordServe, eCharts, auth, $filter, $q, $timeout, toastr, $uibModal, select, confirm) {
    ...
    vm.markEOpt1 = eCharts.markOpt1(eChartCol, 1);
    // 开始手术
    ...
    function outOper() {
        ...
        var drugName = "";
        scope.endOperTime = vm.event.ssjs.occurTime;
        $uibModal.open({
            animation: true,
            backdrop: 'static',
            size: 'ua',
    ...
    vm.upMonNum = function(j, yIndex) {
        var numValue = '';
        if (j.data[yIndex].numValue != '' && j.data[yIndex].numValue != null) {
            numValue = Math.round(j.data[yIndex].numValue);
        }else if (j.data[yIndex].numValue != null && j.data[yIndex].numValue.toString() == '0') {
            numValue = 0;
        }
        j.data[yIndex].numValue = j.name != 'TEMP' ? numValue : j.data[yIndex].numValue;
