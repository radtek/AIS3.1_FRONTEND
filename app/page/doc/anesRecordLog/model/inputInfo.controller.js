...
function inputInfo($scope, IHttp, $uibModalInstance, items, $filter, dateFilter, $timeout, auth) {
    ...
    }else if(user.beCode == 'sybx'&&items.type == 1){//本溪输液回调显示参数
        returnParam.key = 'transfusioninIoeventList';
    }
    ...
    function initParam() {
        ...
        $scope.param = {
            ...
            endTime_: (user.beCode != 'qnzrmyy' && user.beCode != 'syzxyy' && user.beCode != 'cshtyy'&& user.beCode != 'llzyyy' && user.beCode != 'yxyy' && user.beCode != 'qnzzyyy') ? $filter('date')(new Date().getTime() + 1800000, 'yyyy-MM-dd HH:mm') : '',
            docId: items.docId,
            inEventId: '',
            docType: 1,
        };
    ...
    var params = {
        subType:user.beCode == 'sybx'?items.type:undefined,
        type: "I"
    }
    ...
    $scope.save = function() {
        ...
        promise = $timeout(function() {
            IHttp.post("operation/saveIoevent", $scope.param).then(function(result) {
                ...
                $scope.cancel = function() {
                    ioWatch();
                    if(user.beCode !='sybx')returnParam.param.subType = undefined;
