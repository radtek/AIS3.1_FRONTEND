...
function anesEvent($scope, IHttp, $uibModalInstance, $q, $timeout, items, $filter, toastr, auth) {
    ...
    $scope.save = function() {
        ...
        if(!$scope.param.code && ($scope.loginUser.beCode === 'yxyy' || $scope.loginUser.beCode === 'sybx')){
                ...
                $scope.btnActive = false;
                return;
        }else if($scope.loginUser.beCode !== 'yxyy' && $scope.loginUser.beCode !== 'sybx') {
        ...
        if ($scope.param.code == 9) {
            ...
            IHttp.post("operCtl/updateOuterTime", params).then(function(rs) {
                if (rs.data.resultCode == '1') {
                    returnParams.list = $scope.list = rs.data.anaeseventList;
            ...
            var nowTime = $scope.loginUser.beCode === 'yxyy'? new Date().getTime()+7200000:new Date().getTime();
            if ($scope.param.occurTime > nowTime && ($scope.loginUser.beCode !== 'qnzzyyy' || $scope.param.code !== 5)) {
                if($scope.loginUser.beCode === 'yxyy')
    ...
    $scope.delete = function(row) {
        if (row.code == 1 || row.code == 9) {
            ...
            IHttp.post('operation/deleteByCodeAndDocId', { anaEventId: row.anaEventId, code: row.code, docId: items.docId }).then(function(rs) {
                if (rs.data.resultCode != '1') return;
                returnParams.list = $scope.list = rs.data.resultList;
                initParam();
                $scope.cancel = function() {
