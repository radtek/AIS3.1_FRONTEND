...
function InfoConsCtrl($rootScope, $scope, IHttp, toastr, select, auth, $timeout, confirm) {
    ...
    if ($scope.docInfo.roleType == 'HEAD_NURSE' || $scope.docInfo.roleType == 'ANAES_DIRECTOR' || $scope.docInfo.roleType == 'ANAES_DOCTOR') {
    ...
    function fn_save(processState, state) {
        ...
        if ($scope.accedeItem.anaesAssistMeasureList.length == 0) {
            $scope.accedeItem.anaesAssistMeasureList = [];
        }
