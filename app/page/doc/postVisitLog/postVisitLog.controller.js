PostVisitLogCtrl.$inject = ['$rootScope', '$scope', 'IHttp', '$window', '$q', '$state', '$timeout', 'confirm', 'select', 'toastr', 'auth', '$filter'];

module.exports = PostVisitLogCtrl;

function PostVisitLogCtrl($rootScope, $scope, IHttp, $window, $q, $state, $timeout, confirm, select, toastr, auth, $filter) {
    var promise,
        ...
        if ($scope.postFollowGeneralInfo && $scope.postFollowGeneralInfo.length > 0) {
            for (var i = 0; i < $scope.postFollowGeneralInfo.length; i++) {
                if ($scope.postFollowGeneralInfo[i].observeTime)
                    $scope.postFollowGeneralInfo[i].observeTime = $filter('date')(new Date($scope.postFollowGeneralInfo[i].observeTime), 'yyyy-MM-dd HH:mm');
            }
        } else {
        ...
        if ($scope.postFollowSpinalInfo && $scope.postFollowSpinalInfo.length > 0) {
            for (var i = 0; i < $scope.postFollowSpinalInfo.length; i++) {
                if ($scope.postFollowSpinalInfo[i].observeTime)
                    $scope.postFollowSpinalInfo[i].observeTime = $filter('date')(new Date($scope.postFollowSpinalInfo[i].observeTime), 'yyyy-MM-dd HH:mm');
            }
        } else {
            for (var a = 0; a < rows; a++) {
        ...
        if ($scope.postFollowAnalgesicInfo && $scope.postFollowAnalgesicInfo.length > 0) {
            for (var i = 0; i < $scope.postFollowAnalgesicInfo.length; i++) {
                if ($scope.postFollowAnalgesicInfo[i].observeTime)
                    $scope.postFollowAnalgesicInfo[i].observeTime = $filter('date')(new Date($scope.postFollowAnalgesicInfo[i].observeTime), 'yyyy-MM-dd HH:mm');
            }
        } else {
