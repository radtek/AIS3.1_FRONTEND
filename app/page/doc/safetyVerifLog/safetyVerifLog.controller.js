...
function SafetyVerifLogCtrl($rootScope, $scope, IHttp, select, $uibModal, toastr, auth, $timeout, confirm) {
    ...
    function save(processState, type) {
        ...
        if (processState == 'END') {
            if  (processState === 'END' && (!$scope.safeCheck.anesthetistIdList.length > 0 || !$scope.safeCheck.circunurseIdList.length > 0 || !$scope.safeCheck.operatorIdList.length > 0 || (!$scope.exitOperSafeCheck.whereabouts && $scope.exitOperSafeCheck.other1.length <= 0)))  {
