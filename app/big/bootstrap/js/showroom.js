...
function addRow(res){
	...
	for(var i=0;i<rowLength;i++){
		...
		for(var j=0;j<res.columnStyle.length;j++){
			...
			if (res.columnStyle[j].columnName == '性别/年龄') {
				tdArray[j].innerHTML = res.resultList[i][res.columnStyle[j].column] + '/' + res.resultList[i]['age'];
			} else if (res.columnStyle[j].columnName == '巡回/洗手') {
				tdArray[j].innerHTML = res.resultList[i][res.columnStyle[j].column] + '/' + res.resultList[i]['instrnurseName'];
			} else {
...
function addTh(thdate,style){
	...
	for(var i=0;i<thlen;i++){
		...
		if(!!thdate[i].style){
			...
			tdArray[i].style.width=(document.documentElement.clientWidth-50)/thlen+"px";
		}
...
var loadData = function(){
	if(stopPage) return;
	var surl=baseUrl+"/basedata/getOperateInfoByInsideScreen";
